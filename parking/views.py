...
def vehicle_entry(request):
    ...
    vehicle_type = Category.objects.all()
    parking_area_no = Category.objects.all()
    parking_charge = Category.objects.all()
    sai = Category.objects.all()
    prasad = Category.objects.values_list('vehicle_limit', flat=True)
    final = []
    for category_instance in sai:
        count_per_category = Add_vehicle.objects.filter(parking_area_no=category_instance,
                                                          status='parked').values('vehicle_type').annotate(
            total=Count('vehicle_type')).values_list('total', flat=True)
    ...
    context = {'vehicle_type': vehicle_type, 'parking_charge': parking_charge,
               'sai': zipped_data, 'prasad': prasad,'vehicle_data': vehicle_data_final,'parking_area_no':parking_area_no,
        }
